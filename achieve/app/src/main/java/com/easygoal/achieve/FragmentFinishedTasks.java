...
public class FragmentFinishedTasks extends Fragment {
	...
	View oldview=null;
    View newview = null;
	com.easygoal.achieve.mListView lv_finishedtasks;
    String Tags="FragmentFinishedTasks";
	 ...
	 @Override  
		...
		View v=inflater.inflate(R.layout.task_subfg_toptab3_closedtasks, container, false);	  
		//DatabaseHelper TdDB=new DatabaseHelper(); 
		  ...
		  lv_finishedtasks=(com.easygoal.achieve.mListView)v.findViewById(R.id.finishedtasks_lv);
		//  TextView textView = ViewFinder.findViewById(R.id.my_textview); 
		 ...
		 cb_tip.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
		  ...
		  TaskData.adapter_finishedtasks = new mcAdapter_tasks(getActivity(),R.layout.lv_finishedtasks,TaskData.cursor_finishedtasks);
		
		  lv_finishedtasks.setAdapter(TaskData.adapter_finishedtasks); 
  	    Log.d(Tags,"lv finishedtasks show data ok ");

		 lv_finishedtasks.setOnDeleteListener(new mListView.OnDeleteListener() {
			 long oldClickTime=0;

			 @Override
			 public void onDelete(int index) {
				 //Log.d("mlistview delete", "del"+index);
				 long task_id = lv_finishedtasks.getAdapter().getItemId(index);
		 ...
		 // lv_finishedtasks.setFocusable(true);
		  //lv_finishedtasks.setItemsCanFocus(true);
		  //lv_finishedtasks.setSelected(true);
		 /* final Bundle bundle=null;
		  ...
		  lv_finishedtasks.setOnItemClickListener(new OnItemClickListener(){
				...
				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
					// TODO Auto-generated method stub
					try{ 
		                 TaskData.selTaskID=String.valueOf(TaskData.cursor_finishedtasks.getString(TaskData.cursor_finishedtasks.getColumnIndex(TaskData.TdDB.TASK_ID)));
		                 TaskData.selTaskSN=String.valueOf(TaskData.cursor_finishedtasks.getString(TaskData.cursor_finishedtasks.getColumnIndex(TaskData.TdDB.TASK_SN)));
						 new TaskUtils(getActivity(),"s3").showPopupWindow(getActivity(),view);
	            	 }catch(Exception e){
		 ...
		 swipeRefreshLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
			 ...
			 public void onRefresh() {
				 if (TaskData.adapter_finishedtasks!=null&&TaskData.adapter_finishedtasks.getCursor().getCount()>0) {
					 TaskData.adapter_finishedtasks.notifyDataSetChanged();
	...
	void clear(){
		...
		builder.setPositiveButton("确认", new DialogInterface.OnClickListener(){
			...
			public void onClick(DialogInterface arg0, int arg1) {
				...
				Log.d(Tags,"clear"+String.valueOf(TaskData.cursor_finishedtasks.getCount())+"position"+String.valueOf(TaskData.cursor_finishedtasks.getPosition()));
				TaskData.selTaskID=null;
			}
		} );
		builder.setNegativeButton("取消", new DialogInterface.OnClickListener(){
